...
public class EstanciaService {
    ...
    public EstanciaEntity crearEstancia(EstanciaEntity in) {
        if (in == null) throw new IllegalArgumentException("Entidad Estancia es obligatoria");
        ...
        EstudianteEntity estudiante = estudianteRepo.findById(in.getEstudianteArrendador().getId())
                .orElseThrow(() -> new IllegalArgumentException("Estudiante no encontrado con ID: " + in.getEstudianteArrendador().getId()));

        ViviendaEntity vivienda = viviendaRepo.findById(in.getViviendaArrendada().getId())
                .orElseThrow(() -> new IllegalArgumentException("Vivienda no encontrada con ID: " + in.getViviendaArrendada().getId()));
    ...
    public EstanciaEntity obtenerPorId(Long id) {
        return estanciaRepo.findById(id).orElseThrow(() -> new IllegalArgumentException("Estancia no encontrada con ID: " + id));
    }
    ...
    public EstanciaEntity actualizar(Long id, EstanciaEntity updates) {
        ...
        if (updates.getViviendaArrendada() != null && updates.getViviendaArrendada().getId() != null) {
            ...
            if (!nuevaViviendaId.equals(found.getViviendaArrendada().getId())) {
                ...
                if (found.getContrato() != null) {
                    throw new IllegalStateException("No se permite cambiar viviendaArrendada cuando la estancia tiene contrato asociado");
                }
                ViviendaEntity nueva = viviendaRepo.findById(nuevaViviendaId)
                        .orElseThrow(() -> new IllegalArgumentException("Vivienda nueva no encontrada con ID: " + nuevaViviendaId));
        ...
        if (updates.getTiempoEstancia() != null) {
            if (updates.getTiempoEstancia() < 1) throw new IllegalArgumentException("tiempoEstancia debe ser >= 1");
            if (updates.getTiempoEstancia() > 36) throw new IllegalArgumentException("tiempoEstancia no debe exceder 36 meses");
